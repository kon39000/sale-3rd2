...
export default function Header() {
  ...
  return (
    <header className="bg-black/95 backdrop-blur supports-[backdrop-filter]:bg-black/75 border-b border-zinc-800 fixed top-0 left-0 right-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <div className="flex-shrink-0">
            <h1 className="text-2xl font-bold text-amber-400 font-['Pacifico']">フリ校Udemy</h1>
          </div>
          
          <nav className="hidden md:block">
            <div className="ml-10 flex items-baseline space-x-8">
              <Link href="/diagnosis" className="text-gray-200 hover:text-amber-400 px-3 py-2 text-sm font-medium transition-colors">
                講座診断
              </Link>
              <Link href="#courses" className="text-gray-200 hover:text-amber-400 px-3 py-2 text-sm font-medium transition-colors">
                コース一覧
              </Link>
              <Link href="#reviews" className="text-gray-200 hover:text-amber-400 px-3 py-2 text-sm font-medium transition-colors">
                受講生の声
              </Link>
              <Link href="#faq" className="text-gray-200 hover:text-amber-400 px-3 py-2 text-sm font-medium transition-colors">
                FAQ
              </Link>
              <Link href="#apply" className="bg-amber-400 text-black px-4 py-2 rounded-lg text-sm font-medium hover:bg-amber-500 transition-colors whitespace-nowrap cursor-pointer">
                今すぐ申し込む
              </Link>
            </div>
          </nav>
